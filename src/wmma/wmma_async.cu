...
size_t initWmmaAsync() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaAsyncKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

void wmmaAsync(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t shmem_max_size = initWmmaAsync();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_STRIDE * BLOCK_COLS));

    wmmaAsyncKernel<<<grid, block, shmem_max_size>>>(A, B, C, M, N, K);
}
