...
#pragma unroll
    for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
        size_t B_shmem_idx = shmem_store_off + B_shmem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * WMMA_N;
        const half *B_tile_ptr = &shmem[B_shmem_idx][0];
...
size_t initWmmaAsyncPg2sPs2r() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaAsyncPg2sPs2rKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

void wmmaAsyncPg2sPs2r(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t shmem_max_size = initWmmaAsyncPg2sPs2r();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_STRIDE * BLOCK_COLS));

    wmmaAsyncPg2sPs2rKernel<<<grid, block, shmem_max_size>>>(A, B, C, M, N, K);
}
