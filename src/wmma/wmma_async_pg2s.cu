...
#pragma unroll
            for (size_t i = 0; i < WARP_COL_TILES; ++i) {
                ...
                wmma::load_matrix_sync(A_frag[i], A_tile_ptr, AB_SHMEM_STRIDE);
            }
...
#pragma unroll
            for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
                ...
                wmma::load_matrix_sync(B_frag[j], B_tile_ptr, AB_SHMEM_STRIDE);
            }
...
size_t initWmmaAsyncPg2s() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaAsyncPg2sKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

void wmmaAsyncPg2s(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t shmem_max_size = initWmmaAsyncPg2s();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_STRIDE * BLOCK_COLS));

    wmmaAsyncPg2sKernel<<<grid, block, shmem_max_size>>>(A, B, C, M, N, K);
}
