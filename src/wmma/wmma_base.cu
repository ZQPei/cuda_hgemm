...
size_t initWmmaBase() {
    ...
    size_t shmem_max_size = std::max((BLOCK_ROWS + BLOCK_COLS) * (CHUNK_K * WMMA_K) * sizeof(half),
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(wmmaBaseKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

void wmmaBase(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t shmem_max_size = initWmmaBase();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_STRIDE * BLOCK_COLS));

    wmmaBaseKernel<<<grid, block, shmem_max_size>>>(A, B, C, M, N, K);
}
