...
__global__ void mmaAsyncPg2sPs2rKernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C,
    ...
    const size_t shmem_cache_off = BLOCK_ROWS + BLOCK_COLS;
...
#pragma unroll
    ...
    size_t A_shmem_idx = shmem_store_off + BLOCK_ROWS / WARPS_PER_BLOCK * warp_id;
...
#pragma unroll
    ...
    size_t B_shmem_idx = shmem_store_off + B_shmem_idx_off + BLOCK_COLS / WARPS_PER_BLOCK * warp_id;
...
#pragma unroll
            for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
                size_t B_shmem_idx =
                    shmem_load_off + B_shmem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * MMA_N;
                uint32_t B_shmem_lane_addr = __cvta_generic_to_shared(
                    &shmem[B_shmem_idx + lane_id % 8]
...
#pragma unroll
    for (size_t i = 0; i < WARP_COL_TILES; ++i) {
        size_t A_shmem_idx = shmem_store_off + (warp_id / BLOCK_ROW_WARPS) * WARP_ROWS + i * MMA_M;
        uint32_t A_shmem_lane_addr =
...
#pragma unroll
    for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
        size_t B_shmem_idx = shmem_store_off + B_shmem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * MMA_N;
        uint32_t B_shmem_lane_addr = __cvta_generic_to_shared(
            &shmem[B_shmem_idx + lane_id % 8]
                  [(((lane_id / 8) % 2) * 8 +
                    (lane_id % 8 % (PERMUTED_COLS * SHMEM_BANK_ROWS)) / SHMEM_BANK_ROWS * PERMUTED_OFFSET) %
...
#pragma unroll
        for (size_t j = 0; j < WARP_ROW_TILES; ++j) {
            size_t B_shmem_idx =
                shmem_store_off + B_shmem_idx_off + (warp_id % BLOCK_ROW_WARPS) * WARP_COLS + j * MMA_N;
            uint32_t B_shmem_lane_addr = __cvta_generic_to_shared(
                &shmem[B_shmem_idx + lane_id % 8]
                      [(k_step * MMA_K + ((lane_id / 8) % 2) * 8 +
                        (lane_id % 8 % (PERMUTED_COLS * SHMEM_BANK_ROWS)) / SHMEM_BANK_ROWS * PERMUTED_OFFSET) %
...
size_t initMmaAsyncPg2sPs2r() {
    ...
    size_t shmem_max_size = std::max((BLOCK_ROWS + BLOCK_COLS) * AB_SHMEM_STRIDE * sizeof(half) * 2,
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(mmaAsyncPg2sPs2rKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

void mmaAsyncPg2sPs2r(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t shmem_max_size = initMmaAsyncPg2sPs2r();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    mmaAsyncPg2sPs2rKernel<<<grid, block, shmem_max_size>>>(A, B, C, M, N, K);
}
