...
__global__ void mmaAsyncStage4V2Kernel(const half *__restrict__ A, const half *__restrict__ B, half *__restrict__ C) {
...
size_t initMmaAsyncStage4V2() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(mmaAsyncStage4V2Kernel<M,N,K>, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

template<size_t M, size_t N, size_t K>
void _mmaAsyncStage4V2(half *A, half *B, half *C) {
    static size_t shmem_max_size = initMmaAsyncStage4V2<M,N,K>();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    mmaAsyncStage4V2Kernel<M,N,K><<<grid, block, shmem_max_size>>>(A, B, C);
}

void mmaAsyncStage4V2(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    #define IF_STAT if (false)
    #define ELIF_STAT(m, n, k) else if ((m) == M && (n) == N && (k) == K) _mmaAsyncStage4V2<(m), (n), (k)>(A, B, C)
    #define ELSE_STAT else { printf("NOT_IMPLEMENTED\n"); __builtin_trap(); }

    // 256 512 768 1024 1536 2048 3072 4096 5120 6144 7168 8192 9216 10240 11264 12288 13312 14336 15360 16384
    IF_STAT;
    ELIF_STAT(256, 256, 256);
    ELIF_STAT(512, 512, 512);
    ELIF_STAT(1024, 1024, 1024);
    ELIF_STAT(2048, 2048, 2048);
    ELIF_STAT(4096, 4096, 4096);
    ELIF_STAT(8192, 8192, 8192);
    ELIF_STAT(10240, 10240, 10240);
    ELSE_STAT;
