...
size_t initMmaAsyncPg2s() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(mmaAsyncPg2sKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

void mmaAsyncPg2s(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t shmem_max_size = initMmaAsyncPg2s();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    mmaAsyncPg2sKernel<<<grid, block, shmem_max_size>>>(A, B, C, M, N, K);
}
