...
size_t initMmaPermuted() {
    ...
    HGEMM_CHECK_CUDART_ERROR(
        cudaFuncSetAttribute(mmaPermutedKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, shmem_max_size));

    return shmem_max_size;
}

void mmaPermuted(half *A, half *B, half *C, size_t M, size_t N, size_t K) {
    static size_t shmem_max_size = initMmaPermuted();

    dim3 block(THREADS_PER_BLOCK);
    dim3 grid(BLOCK_STRIDE, div_ceil(M, BLOCK_ROWS), div_ceil(N, BLOCK_COLS * BLOCK_STRIDE));

    mmaPermutedKernel<<<grid, block, shmem_max_size>>>(A, B, C, M, N, K);
}
