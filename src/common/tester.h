...
#ifndef __CUDA_HGEMM_TESTER_H__
#define __CUDA_HGEMM_TESTER_H__
...
public:
    explicit Tester(size_t M = 512, size_t N = 2048, size_t K = 1024, size_t warmup_iterations = 1,
        ...
        if (m_enable_check) {
            m_cuda_timer.start();
            cublas_tensor_op(m_A->getGpuData(), m_B->getGpuData(), m_base->getGpuData(), m_M, m_N, m_K);
    ...
    void evaluate(Func &&hgemm, const std::string &name) {
        ...
        for (size_t i = 0; i < m_warmup_iterations; ++i) {
            hgemm(m_A->getGpuData(), m_B->getGpuData(), m_C->getGpuData(), m_M, m_N, m_K);
        }
        m_warmup_time = double(m_cuda_timer.end()) / double(m_warmup_iterations);
...
private:
    ...
    void profile(Func &&hgemm, const std::string &name) {
        ...
        m_profiling_time = double(m_cuda_timer.end()) / double(m_profiling_iterations);
        m_throughput = double(m_M * m_N * m_K * 2) * 1e-12 / (double(m_profiling_time) * 1e-3);
